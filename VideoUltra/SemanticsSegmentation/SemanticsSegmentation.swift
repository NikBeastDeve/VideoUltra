...
@available(iOS 15.0, *)
public struct SemanticsSegmentation {
    
    public init() {
    }
    
    var personSegmentationRequest = VNGeneratePersonSegmentationRequest()
    var faceRectangleRequest = VNDetectFaceRectanglesRequest()
    var humanRectanglesRequest:VNDetectHumanRectanglesRequest = {
    ...
    var segmentationRequest: VNCoreMLRequest? =  {
        let stringPath = Bundle.main.path(forResource: "segmentation", ofType: "mlmodelc")
        print(stringPath)
        let url = Bundle.main.url(forResource: "segmentation", withExtension: "mlmodelc")
